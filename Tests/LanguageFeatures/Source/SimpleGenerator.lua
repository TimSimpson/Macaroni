function printMem(blah)
    local mem = collectgarbage("count");
    print("MEMORY " .. blah .. ":" .. mem);
...
local VariableAssignment = Macaroni.Model.Cpp.VariableAssignment;

printMem("Local module aliasing.");
...
function parseNamespace(node, path)
    printMem("parseNamespace");
    assert(node.Member.TypeName == TypeNames.Namespace);    
    printMem("parseNamespace 2");
    path:CreateDirectory();
    printMem("parseNamespace 3");
    iterateNodes(node.Children, path);
    printMem("parseNamespace 4");
end
